...
let slider_about = new Swiper('.specialties__sliders', {
	slideClass: "body-specialties",
	observer: true,
	observeParents: true,
	...
	breakpoints: {
		...
		992: {
			...
			mousewheel: {
				sensitivity: 1,
				eventsTarget: ".specialties__body",
			},
		},
		...
		for (let index = 0; index < inputs.length; index++) {
			...
			if (input.classList.contains('_date')) {
				const calendarItem = datepicker(input, {
					...
					customMonths: ["Jun", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sept", "Oct", "Nov", "Dec"],
					...
					formatter: (input, date, instance) => {
						...
						const newDate = ` ${mouth} ${dateArray[2]}`;
						input.value = newDate;
...
function mapAdd() {
	...
	tag.src = "https://maps.google.com/maps/api/js?sensor=false&amp;key=AIzaSyAuVbTflCHbpDLXJMdufwMjLmY4om-db94&callback=mapInit";
...
function map(n) {
	...
	function init() {
		...
		var myMap = new ymaps.Map("map", {
			...
			controls: [],
			center: [53.685092, 23.82497],
			// Уровень масштабирования. Допустимые значения:
			// от 0 (весь мир) до 19.
			zoom: 15,
		});

		let myPlacemark = new ymaps.Placemark([53.685092, 23.82497], {
		},{
			// Опции.
